...
**Adendo 21/09 (P38) — o briefing virou peça semanal.** A pedido do dono, o material para as creators mudou de formato: em vez de um briefing analítico, uma peça recorrente **"Os melhores vídeos da semana + banco de ganchos"**. Entregue: `Criativo da Semana Rhode Jeans_2026-09-21.docx` (Drive `1AcS9EG1VDtUc-mPKaTPd12uKe7Al2un9`), com pódio de 8 vídeos da semana 15–21/09 (81 vídeos venderam, 231 peças), 3 exemplos de "vendeu sem viralizar" e **53 ganchos reais** de vídeos que venderam, em 7 temas. Geradores em `relatorios/2026-09/_build_semanal_criativos/` — ver memória `reference_criativo_da_semana_template` para as regras de curadoria.

**Adendo 21/09 (P38) — transcrição dos campeões e o hook rate que não existe.** Pedido: "os 25 melhores vídeos com hook rate 2s e transcritos". Entregue `Relatorio Top 25 Criativos com Transcricao_2026-09-21` ([planilha](https://docs.google.com/spreadsheets/d/12JrsTpZlq6ri9qAFIV52PcgXMgB1tn0jRDt1cpnTehY/edit)).

- ⛔ **Hook rate de 2s não existe em nenhuma fonte nossa** — `shop_videos` dá views/CTR/pedidos/GMV e só; a API do GMV Max rejeita métricas de mídia; e a retenção dos vídeos de creator vive no Studio delas. Substitutos entregues: **o texto falado nos 2 primeiros segundos** e o **CTR** por vídeo.
- ✅ **Transcrição sem ASR:** o TikTok gera legenda automática e o `yt-dlp` puxa (faixa `por-PT`). Sem sessão logada, o TikTok bloqueia consulta em série: **10 de 25** mesmo com 8–14 s entre chamadas e 3 passadas. Cobertura alta exigiria `--cookies-from-browser chrome` → depende de autorização do dono.
- **Padrão dos ganchos campeões:** vocativo direto ("amiga", "gente", "cês") + diferencial no primeiro fôlego, sem introdução. Ex.: *"cês tão vendo como ela é mais sequinha aqui na barra"* (183 peças, CTR 6,42%).
